...
void Task::initialize(Field &current_Field, vector <Robot *> &Robots,vector <Programm *> &Programms) {
   ...
   for (int i = 0; i < count_commands; i++) {
     ...
     /* for (auto it = Programms.begin(); it < Programms.end(); it++) {
         cout <<"цвет ---"<< (*it)->get_col()<<"--- цвет2 ---"<< color_prog[f_color] << endl;
         
         if( (*it)->get_col() == color_prog[f_color])
            num=1;
   // it->doSomething ();
}  
  ...
  {
     cout <<"цвет ---"<< Programms[i]->get_col()<<"--- цвет2 ---"<< color_prog[f_color] << endl;
  if( Programms[i]->get_col() == color_prog[f_color])
            num=1;
      ...
      if (it != Programms.end())
      {
         ...
         (*it)->add(command);
         //Programms[int(*it)]->add(command);
         }
      else{
         cout << "Не нашёл"  << endl;
            Programm * new_programm = new Programm(color_prog[f_color]);
         _abracadabra_cast(*(new_programm));